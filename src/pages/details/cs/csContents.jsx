...
export const csContents = [
    ...
    text: {
      p1: (
        <p>
          ...
          functions or methods). In many languages, the class name is used as
          the name for the class (the template itself), the name for the default
          constructor of the class (a subroutine that creates objects), and as
          the type of objects generated by instantiating the class; these
          distinct concepts are easily conflated. Although, to the point of
          conflation, one could argue that is a feature inherent in a language
          because of its polymorphic nature and why these languages are so
          powerful, dynamic and adaptable for use compared to languages without
          polymorphism present. Thus they can model dynamic systems (i.e. the
          real world, machine learning, AI) more easily.
        </p>
      ),
    },
  },
  {
    ...
    anchor: "Polymorphism_(computer_science)",
    text: {
      p1: (
        <p>
  ...
  },
  {
    ...
    isLargerTitle: false,
    path: "/cs",
    url: `${wikiUrl}/Function_prototype`,
    anchor: "Function_prototype",
    text: {
      p1: (
        <p>
          In computer programming, a function prototype or function interface is
          a declaration of a function that specifies the function’s name and
          type signature (arity, data types of parameters, and return type), but
          omits the function body. While a function definition specifies how the
          function does what it does (the "implementation"), a function
          prototype merely specifies its interface, i.e. what data types go in
          and come out of it. The term "function prototype" is particularly used
          in the context of the programming languages C and C++ where placing
          forward declarations of functions in header files allows for splitting
  ...
  },
  {
    ...
    isLargerTitle: false,
    path: "/cs",
    url: `${wikiUrl}/Uniform_Resource_Identifier`,
    anchor: "Uniform_Resource_Identifier",
    text: {
      p1: (
        <p>
          ...
          information resources such as web pages and books. Some URIs provide a
          means of locating and retrieving information resources on a network
          (either on the Internet or on another private network, such as a
          computer filesystem or an Intranet); these are Uniform Resource
          Locators (URLs). A URL provides the location of the resource. A URI
          identifies the resource by name at the specified location or URL.
          Other URIs provide only a unique name, without a means of locating or
          retrieving the resource or information about it, these are Uniform
          Resource Names (URNs). The web technologies that use URIs are not
          limited to web browsers. URIs are used to identify anything described
          using the Resource Description Framework (RDF), for example, concepts
          that are part of an ontology defined using the Web Ontology Language
          (OWL), and people who are described using the Friend of a Friend
          vocabulary would each have an individual URI.
        </p>
      ),
    },
  },
    ...
    text: {
      p1: (
        <p>
          ...
          computer network and a mechanism for retrieving it. A URL is a
          specific type of Uniform Resource Identifier (URI), although many
          people use the two terms interchangeably. URLs occur most commonly to
  ...
  },
  {
    ...
    isLargerTitle: false,
    path: "/cs",
    url: `${wikiUrl}/System_resource`,
    anchor: "System_resource",
    text: {
      p1: (
        <p>
          ...
          Virtual system resources include files (concretely file handles),
          network connections (concretely network sockets), and memory areas.
        </p>
      ),
    },
  },
  {
    ...
    isLargerTitle: false,
    path: "/cs",
    url: `${wikiUrl}/Computer_network`,
    anchor: "Computer_network",
    text: {
      p1: (
        <p>
          ...
          other. These interconnections are made up of telecommunication network
  ...
  },
  {
    ...
    isLargerTitle: false,
    path: "/cs",
    url: `${wikiUrl}/Linked_data`,
    anchor: "Linked_data",
    text: {
      p1: (
        <p>
          In computing, linked data (often capitalized as Linked Data) is
          structured data which is interlinked with other data so it becomes
          more useful through semantic queries. It builds upon standard Web
          technologies such as HTTP, RDF and URIs, but rather than using them to
          serve web pages only for human readers, it extends them to share
          information in a way that can be read automatically by computers. Part
          of the vision of linked data is for the Internet to become a global
          database.
        </p>
      ),
    },
    ...
    text: {
      p1: (
        <p>
          ...
          commonly used for developing user interfaces that divide the related
          program logic into three interconnected elements. This is done to
    ...
    text: {
      p1: (
        <p>
          ...
          principles intended to make object-oriented designs more
          understandable, flexible, and maintainable. The principles are a
          subset of many principles promoted by American software engineer and
          instructor Robert C. Martin, first introduced in his 2000 paper Design
  ...
  },
  {
    ...
    isLargerTitle: false,
    path: "/cs",
    url: `${wikiUrl}/Field_(computer_science)`,
    anchor: "Field_(computer_science)",
    text: {
      p1: (
        <p>
          In computer science, data that has several parts, known as a record,
          can be divided into fields (data fields). Relational databases arrange
          data as sets of database records, so called rows. Each record consists
          of several fields; the fields of all records form the columns.
    ...
    text: {
      p1: (
        <p>
          ...
          (see object-oriented program architecture), is-a (is_a or is a) is a
          subsumption relationship between abstractions (e.g. types, classes),
          wherein one class A is a subclass of another class B (and so B is a
          superclass of A). In other words, type A is a subtype of type B when
          A's specification implies B's specification. That is, any object (or
          class) that satisfies A's specification also satisfies B's
          specification, because B's specification is weaker.
        </p>
      ),
    },
  },
  ...
  },
  {
    ...
    isLargerTitle: false,
    path: "/cs",
    url: `${wikiUrl}/Initialization_(programming)`,
    anchor: "Initialization_(programming)",
    text: {
      p1: (
        <p>
          In computer programming, initialization (or initialisation) is the
          assignment of an initial value for a data object or variable. The
          manner in which initialization is performed depends on the programming
          language, as well as the type, storage class, etc., of an object to be
          initialized. Programming constructs which perform initialization are
          typically called initializers and initializer lists. Initialization is
          distinct from (and preceded by) declaration, although the two can
          sometimes be conflated in practice. The complement of initialization
          is finalization, which is primarily used for objects, but not
          variables.
        </p>
      ),
    },
  },
  {
    ...
    text: {
      p1: (
        <p>
          In computer programming, specifically object-oriented programming, a
          class invariant (or type invariant) is an invariant used for
          constraining objects of a class. Methods of the class should preserve
          the invariant. The class invariant constrains the state stored in the
          object.
        </p>
      ),
    },
  },
  {
    ...
    isLargerTitle: false,
    path: "/cs",
    url: `${wikiUrl}/Mutator_method`,
    anchor: "Mutator_method",
    text: {
      p1: (
        <p>
          In computer science, a mutator method is a method used to control
          changes to a variable. They are also widely known as setter methods.
          Often a setter is accompanied by a getter (together also known as
          accessors), which returns the value of the private member variable.
        </p>
      ),
    },
  },
  ...
  },
  {
    ...
    isLargerTitle: false,
    path: "/cs",
    url: `${wikiUrl}/Time_complexity`,
    anchor: "Time_complexity",
    text: {
      p1: (
        <p>
          In computer science, the time complexity is the computational
          complexity that describes the amount of computer time it takes to run
          an algorithm. Time complexity is commonly estimated by counting the
          ...
          perform. Thus, the amount of time taken and the number of elementary
          operations performed by the algorithm are taken to be related by a
          constant factor.
        </p>
      ),
    },
  },
  {
    ...
    isLargerTitle: false,
    path: "/cs",
    url: `${wikiUrl}/Computational_complexity`,
    anchor: "Computational_complexity",
    text: {
      p1: (
        <p>
          In computer science, the computational complexity or simply complexity
          of an algorithm is the amount of resources required to run it.
          Particular focus is given to computation time (generally measured by
          the number of needed elementary operations) and memory storage
          requirements. The complexity of a problem is the complexity of the
  ...
  },
  {
    ...
    isLargerTitle: false,
    path: "/cs",
    url: `${wikiUrl}/Space_complexity`,
    anchor: "Space_complexity",
    text: {
      p1: (
        <p>
          The space complexity of an algorithm or a computer program is the
          amount of memory space required to solve an instance of the
          computational problem as a function of characteristics of the input.
          It is the memory required by an algorithm until it executes
          completely.
        </p>
      ),
    },
  },
  {
    ...
    isLargerTitle: false,
    path: "/cs",
    url: `${wikiUrl}/Information_retrieval`,
    anchor: "Information_retrieval",
    text: {
      p1: (
        <p>
  ...
  },
  {
    ...
    isLargerTitle: false,
    path: "/cs",
    url: `${wikiUrl}/Best_worst_and_average_case`,
    anchor: "Best_worst_and_average_case",
    text: {
      p1: (
        <p>
          In computer science, best, worst, and average cases of a given
          algorithm express what the resource usage is at least, at most and on
          average, respectively. Usually the resource being considered is
          running time, i.e. time complexity, but could also be memory or some
          other resource. Best case is the function which performs the minimum
          number of steps on input data of n elements. Worst case is the
          function which performs the maximum number of steps on input data of
          size n. Average case is the function which performs an average number
  ...
  {
    id: uuidv4(),
    title: "Runtime (program lifecycle phase)",
    isLargerTitle: false,
    path: "/cs",
    url: `${wikiUrl}/Runtime_(program_lifecycle_phase)`,
    anchor: "Runtime_(program_lifecycle_phase)",
    text: {
      p1: (
        <p>
